...
def connection_scan(target_ip, target_port):
    ...
    try:
        ...
        conn_socket.connect((target_ip, target_port)) # Connecting socket to target
        conn_socket.send(b'Banner_query\r\n') # Banner request
        results = conn_socket.recv(100) # grab the banner results
        print("[+] {}/tcp open".format(target_port))
        print('[+] {}'.format(str(results))) # Print banner grabbed results
    except OSError:
